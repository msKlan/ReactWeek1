...
function apiFacade() {
  ...
  function addEditPerson(person) {
   //Complete me. A smart version will handle both Add and Edit, but focus on Add (POST) only first
    console.log('addEditPerson1:', makeOptions('POST', person));
    const method = person.id ? 'PUT' : 'POST';
    const idParam = person.id ? '/' + person.id : '';
    console.log(method);
    return fetch(URL + idParam, makeOptions(method, person)).then(handleHttpErrors);
  }
